...
{
  /**
   * this comes with it's own catch 22 bro, this is essentially the new page component reporposed, for all the same valid
   * reasons I would not be trying my ass off to refactor kankan, I go just pass props chop am, the idea is simple, essentially when you edit you
   * are doing the same operation as a c
   */
}
...
import {
  PortableText,
  type PortableTextBlock as PortableTextReactTextBlock,
} from "@portabletext/react";
import useUpdateArticle from "@/hooks/blogs/useUpdateSubmittedArticle";
...
/**
 *
 * Bug here was that images coming from sanity don't show up in the edit as images they show as refereneces so the goal of the transform is to handle that
 */

const transformSanityImagesToDirectUrls = (
  ...
  return content.map((block) => {
    if (block._type === "image") {
      if ("asset" in block && block.asset) {
        return {
          ...block,
          src: urlFor(block.asset).quality(90).url(),
...
const EditArticlePublishedByAuthor = ({ blogPost }: { blogPost: BlogPost }) => {
  const articleDetailsFromBlogPost = {
    title: blogPost.title!,
    excerpt: blogPost.excerpt!,
    content: transformSanityImagesToDirectUrls(
      blogPost.content! as PortableTextBlock[]
    ),
    isDraft: true,
    posterImage: {
      src: urlFor(blogPost.featuredImage!.asset).width(1230).height(600).url(),
      alt: blogPost.featuredImage?.alt || "blog image",
    },
    createdAt: blogPost.createdAt,
  };
  const [article, setArticle] = useState<Article>(articleDetailsFromBlogPost!);
  const DRAFT_KEY = `blog-editor-mode-${blogPost.slug.current}`;
  ...
  const loadDraftFromStorage = (): Article | null => {
    try {
      ...
      if (savedDraft) {
        ...
        // Transform any Sanity images that might be in the saved draft from that bug I mentioned at the top
        parsed.content = transformSanityImagesToDirectUrls(
          parsed.content || []
        );
        return parsed;
      }
  ...
  useEffect(() => {
    ...
    const getAuthorExtraNeeds = async () => {
      ...
      const authorDatabaseReferenceId = blogPost.authorDatabaseReferenceId;
  ...
  const { updateArticle, isUpdating, error, isSuccess } = useUpdateArticle();
  ...
  useEffect(() => {
    ...
    if (isUpdating) {
      ...
      toastId = toast.loading("Updating article...", {
        position: "top-center",
        closeOnClick: false,
        pauseOnHover: false,
        draggable: false,
            ...
            <div className="flex items-center justify-between py-3 sm:py-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <button
                  ...
                  disabled={!isPublishReady || isUpdating}
                  className={`px-3 sm:px-6 py-2 bg-blue-500 text-white hover:bg-blue-600 transition-all duration-200 font-light tracking-wide text-sm sm:text-base shadow-sm ${!isPublishReady || isUpdating ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  {isUpdating ? (
...
export default EditArticlePublishedByAuthor;
