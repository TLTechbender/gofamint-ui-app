...
export const applyForAuthor = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    if (user.author) {
      ...
      if (!isDeleted) {
        if (status === "APPROVED") {
          throw new AppError(
            "You are already an approved author",
            400,
    ...
    try {
      ...
      sanityImageAsset = await sanityClient.assets.upload(
        "image",
      ...
      throw new AppError(
        ...
        500,
        [{ profilePic: "Image upload failed" }]
      );
    }
      ...
      newAuthor = await prisma.$transaction(async (tx) => {
        ...
        const author = await tx.author.create({
          data: {
            userId,
            authorBio: bio,
            profilePic: sanityImageAsset.url,
