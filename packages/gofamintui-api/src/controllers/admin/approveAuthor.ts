// controllers/admin/approveAuthor.ts
...
export const approveAuthor = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    const validationResult = approveAuthorSchema.safeParse(req.body);
    if (!validationResult.success) {
      const formattedErrors = formatZodErrors(validationResult.error);
      throw new AppError('Validation failed', 400, formattedErrors);
    }

    const { authorId, notes } = validationResult.data;
    ...
    return AppResponse(
      ...
      {
        author: {
          id: approvedAuthor.id,
          userId: approvedAuthor.userId,
