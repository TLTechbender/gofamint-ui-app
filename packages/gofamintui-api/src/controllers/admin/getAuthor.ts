  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const response = {
      ...
      statistics: {
        ...
        totalViews: author.blogs.reduce(
          (sum, blog) => sum + blog.verifiedViewCount,
          0
        ),
        totalLikes: author.blogs.reduce(
          (sum, blog) => sum + blog._count.likes,
          0
        ),
        totalComments: author.blogs.reduce(
